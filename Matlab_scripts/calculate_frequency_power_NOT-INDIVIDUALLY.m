...
for ix = 1:ns;	% for each i-th file
    ...
    if isempty(k1(1)) && isempty(k2(1));
        ...
        data_struct(rownumber).IAF=10; % 8 - 12 hz
        ...
        data_struct(rownumber).ThetaFreq=6; % 4-8Hz
        ...
        [d, Theta2ind]=min( abs(ff-(data_struct(rownumber).ThetaFreq+2)));
        ...
        [d, Alpha_1]=min( abs(ff-(data_struct(rownumber).IAF-2))); %  ALPHA, e.g. 8-12
